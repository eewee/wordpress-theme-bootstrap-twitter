        ...
        <section id="primary" class="span8">
			...
			<?php if ( have_posts() ) : ?>
				...
				<header class="page-header">
					<h1 class="page-title">
						<?php if ( is_day() ) : ?>
							<?php printf( __( 'Daily Archives: %s', 'eewee' ), '<span>' . get_the_date() . '</span>' ); ?>
						<?php elseif ( is_month() ) : ?>
							<?php printf( __( 'Monthly Archives: %s', 'eewee' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'eewee' ) ) . '</span>' ); ?>
						<?php elseif ( is_year() ) : ?>
							<?php printf( __( 'Yearly Archives: %s', 'eewee' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'eewee' ) ) . '</span>' ); ?>
						<?php else : ?>
							<?php _e( 'Blog Archives', 'eewee' ); ?>
						<?php endif; ?>
					</h1>
				</header>
